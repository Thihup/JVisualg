...
public class VisualgLanguageServer implements LanguageServer, LanguageClientAware, WorkspaceService, TextDocumentService {
    ...
    public CompletableFuture<Either<List<CompletionItem>, CompletionList>> completion(CompletionParams position) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            Map<String, TypeChecker.Declaration.Function> functions = TypeChecker.DEFAULT_GLOBAL_SCOPE.functions();

            Stream<CompletionItem> defaultFunctions = functions.entrySet().stream().map(func -> {
            ...
            Stream<CompletionItem> keywords = Map.ofEntries(
            ...
            Stream<CompletionItem> snippets = Map.ofEntries(
                    Map.entry("!", """
                            algoritmo "${0:semnome}"
                            inicio
                                $1
                    ...
                    Map.entry("#", """
                            ...
                            // Autor : $2
                            // Data : ${3:%s}
                            """.formatted(LocalDate.now().format(DateTimeFormatter.ofLocalizedDate(FormatStyle.SHORT)))),
            ...
            return Either.forLeft(Stream.of(defaultFunctions, keywords, snippets).flatMap(s -> s).toList());
        });
    }
